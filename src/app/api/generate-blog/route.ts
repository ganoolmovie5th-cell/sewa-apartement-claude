...
// ─────────────────────────────────────────────────────────────────────────────
// Generate konten artikel menggunakan OpenAI (dengan fallback jika tidak ada key)
// ─────────────────────────────────────────────────────────────────────────────
async function generateArticleWithOpenAI(topic: (typeof WEEKLY_TOPICS)[0]) {
...
Format output JSON (HANYA JSON, tanpa markdown):
{
  ...
  "content_id": "konten artikel lengkap bahasa Indonesia dalam format markdown, minimal 400 kata, gunakan heading **bold**, list, dan paragraf yang informatif. Selipkan nama SewaApartement.id secara natural.",
  "content_en": "full article content in English markdown format, minimum 400 words",
  "tags": ["tag1", "tag2", "tag3", "tag4"]
}`;

  // ── Jika ada OpenAI API key → pakai OpenAI ─────────────────────────────
  if (apiKey) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newArticle = {
      ...
      featured: false,
      autoGenerated: true,
      generatedWithAI: !!process.env.OPENAI_API_KEY,
    };
